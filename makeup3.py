...
def fetch_blush_products():
    url = "http://makeup-api.herokuapp.com/api/v1/products.json"
    params = {'product_type': 'blush', 'limit': 25}  # Only lipstick products, limit to 25
    response = requests.get(url, params=params)
    
    if response.status_code == 200:
        return response.json()
    else:
        print(f"Failed to fetch blush products: {response.status_code}")
        return None
...
def create_database():
    ...
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS myblush (
...
def batch_insert_into_database(products):
    ...
    for product in products:
        if product.get('product_type') == 'blush':  # Check product_type before insertion
            # Check if product with the same id already exists in the database
            cursor.execute("SELECT COUNT(*) FROM myblush WHERE id = ?", (product.get('id'),))
            count = cursor.fetchone()[0]

            if count == 0:  # Only insert if product with the same id doesn't exist
                cursor.execute('''
                INSERT INTO myblush (brand_id, brand_name, name, price, currency)
                VALUES (?, ?, ?, ?, ?)
                ''', (
...
def main():
    ...
    conn = sqlite3.connect('makeup.db')
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM myblush")
    ...
    blush_products = fetch_blush_products()
